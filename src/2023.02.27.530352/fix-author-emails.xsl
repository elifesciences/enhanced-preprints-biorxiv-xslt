...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="article[//article-meta/article-version='1.2']//article-meta//contrib[@contrib-type='author' and @corresp='yes']">
        <xsl:choose>
            <xsl:when test="./name/surname='Jiang'">
                <xsl:copy>
                    <xsl:apply-templates select="@*|contrib-id|name"/>
                    <email>jianghui55@163.com</email>
            ...
            <xsl:when test="./name/surname='Li'">
                <xsl:copy>
                    <xsl:apply-templates select="@*|contrib-id|name"/>
                    <email>leways@gwcmc.org</email>
                    <xsl:apply-templates select="*[not(name()=('name','contrib-id')) and not(@ref-type='corresp')]"/>
                </xsl:copy>
            </xsl:when>
            <xsl:when test="./name/surname='Chen'">
                <xsl:copy>
                    <xsl:apply-templates select="@*|contrib-id|name"/>
                    <email>jiayuchen@nju.edu.cn</email>
                    <xsl:apply-templates select="*[not(name()=('name','contrib-id')) and not(@ref-type='corresp')]"/>
                </xsl:copy>
            </xsl:when>
            <xsl:otherwise>
