...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="article[//article-meta/article-version='1.2']//article-meta//contrib[@contrib-type='author' and @corresp='yes']">
        <xsl:choose>
            <xsl:when test="./name/surname='Marqu&#x00E8;s-Bonet'">
                <xsl:copy>
                    <xsl:apply-templates select="@*"/>
                    <xsl:apply-templates select="*[name()=('contrib-id','name')]"/>
                    <email>tomas.marques@upf.edu</email>
                    <xsl:apply-templates select="*[not(name()=('contrib-id','name')) and not(@ref-type='corresp')]"/>
                </xsl:copy>
            </xsl:when>
            <xsl:when test="./name/surname='Vila'">
                <xsl:copy>
                    <xsl:apply-templates select="@*"/>
                    <xsl:apply-templates select="*[name()=('contrib-id','name')]"/>
                    <email>roger.vila@csic.es</email>
                    <xsl:apply-templates select="*[not(name()=('contrib-id','name')) and not(@ref-type='corresp')]"/>
                </xsl:copy>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="@*"/>
                    <xsl:apply-templates select="*[name()=('contrib-id','name')]"/>
                    <email>carles.lalueza.fox@gmail.com</email>
                    <xsl:apply-templates select="*[not(name()=('contrib-id','name')) and not(@ref-type='corresp')]"/>
                </xsl:copy>
            </xsl:otherwise>
        </xsl:choose>
